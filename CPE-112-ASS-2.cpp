...
class cpe1b
	...
	public:
		...
		void wasteful_number(long x);// PASABOT ANI KAY MAGHIMO UG FUNCTION NGA WASTEFUL NUMBER
		long prime(long num); // KANI GAHIMOG FUNCTION NGA GINGANLAN UG PRIME NGA longEGER KAY longEGER MAN ANG PRIME DILI STRING DILI POD FLOAT
		long binary(long num1);// KANI MAO NI AG PAG BINARY SA MGA PRIME NUMBERS 
		long reverse(long num2);//KANI MAO NI AG PAG REVERSE SA MGA BINARY NUMBERS
		long digit(long num3);//KANI MAO NI AG PAG PANGITA SA KUNG PILA ANG DIGIT
		long primefactorization(long num4);//MAO NI AG FACTOR SA PRIME 
		long j,count,x;
...
void cpe1b::wasteful_number(long x)
	{
		cout<<endl<<"WASTEFUL NUMBER NI SIYA"<<endl;
		digit(x);
		if (digits<factors)
			{
			cout<<"WASTERFUL MEE YEEY"<<endl;
			cout<<x;
			}
	}
